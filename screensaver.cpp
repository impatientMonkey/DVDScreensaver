...
//function hides blinking cursor
void hideCursor(){
...
//function shows blinking cursor
void showCursor(){
    HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
    CONSOLE_CURSOR_INFO cursorInfo;
    GetConsoleCursorInfo(hConsole, &cursorInfo);
...
//function sets cursor's position in console window
...
//function for console animation
void animationThreadFunc(const std::vector<std::string>& logo){
    CONSOLE_SCREEN_BUFFER_INFO csbi;
...
int main(){
    ...
    std::thread marquee(animationThreadFunc, std::cref(logo));
